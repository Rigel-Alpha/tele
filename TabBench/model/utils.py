...
def bag_set(exampleset, bag_attr=0, train=True):
    """
...
def modeltype_to_method(model):
    if model == "mlp":
        ...
        return SvmMethod
    elif model == 'contra-knn':
        from model.methods.contra_knn import ContraKNNMethod
        return ContraKNNMethod
    elif model in ['contra-nca','contra-wknn']:
        from model.methods.contra_nca import ContraNCAMethod
        return ContraNCAMethod
    elif model == 'contra-ncm':
        from model.methods.contra_ncm import ContraNCMMethod
        return ContraNCMMethod
    elif model == 'contra-lr':
        from model.methods.contra_lr import ContraLRMethod
        return ContraLRMethod
    else:
