...
class SupervisedContrastiveLoss(nn.Module):
    ...
    def forward(self, projections, targets):
        ...
        # exp_dot_tempered = torch.exp(dot_tempered / self.temperature)
...
class ContraNCAMethod(Method):
    ...
    def construct_model(self, model_config = None):
        from model.models.contra_nca import contra_nca
        if model_config is None:
            model_config = self.args.config['model']
        self.model = contra_nca(
    ...
    def fit(self, N, C, y, info, train = True, config = None):
        ...
        self.criterion2=SupervisedContrastiveLoss(temperature=self.args.config["model"]["temperature"])
    ...
    def train_epoch(self, epoch):
        ...
        for i, (X, y) in enumerate(self.train_loader, 1):
            ...
            if self.N is not None and self.C is not None:
                X = torch.cat([X[0], X[1]],dim=1)
            feature =self.model(X)
            loss = self.criterion2(feature,y)
            tl.add(loss.item())
    ...
    def train_epoch_head(self, epoch):
        self.model.train()
        features,labels=[],[]
        for i, (X, y) in enumerate(self.train_loader, 1):
            self.train_step = self.train_step + 1
    ...
    def validate(self, epoch):
        ...
        for epoch2 in range(self.args.max_epoch):
            self.train_epoch_head(epoch2)
            self.model.eval()
            with torch.no_grad():
                for i, (X, y) in tqdm(enumerate(self.val_loader)):
                    if self.N is not None and self.C is not None:
                        X = torch.cat([X[0], X[1]],dim=1)                           
                    pred,feature =self.model.predict(X)
                    test_logit.append(pred)
                    test_label.append(y)  
            test_logit = torch.cat(test_logit, 0)
            test_label = torch.cat(test_label, 0)
            ...
            if measure(vres[0], best_res) or epoch2 == 0:
                best_res = vres[0]
                best_epoch = epoch2
                val_count =0
            else:
