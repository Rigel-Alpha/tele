...
class ContraNCMMethod(Method):
    ...
    def construct_model(self, model_config = None):
        from model.models.contra_ncm import contra_ncm
        if model_config is None:
            model_config = self.args.config['model']
        self.model = contra_ncm(
    ...
    def fit(self, N, C, y, info, train = True, config = None):
        ...
        self.criterion=SupervisedContrastiveLoss(temperature=self.args.config["model"]["temperature"])
    ...
    def predict(self, N, C, y, info, model_name):
        ...
        with torch.no_grad():
            ...
            centers=[]
            for i in range(self.model.d_out):
                centers.append(torch.mean(features[labels==i],dim=0,keepdim=True))
            self.centers=torch.cat(centers,dim=0)            
            for i, (X, y) in tqdm(enumerate(self.test_loader)):
                if self.N is not None and self.C is not None:
                    X = torch.cat([X[0], X[1]],dim=1)                           
                pred,feature=self.model.predict(X,self.centers)
    ...
    def train_epoch(self, epoch):
        ...
        features=torch.cat(features,0)
        labels=torch.cat(labels,0)
        #caculate center of each class using features and labels
        centers=[]
        for i in range(self.model.d_out):
            centers.append(torch.mean(features[labels==i],dim=0,keepdim=True))
        self.centers=torch.cat(centers,dim=0)
